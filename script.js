...
function findCafes(searchTerm) {
    ...
    return filteredCafes.map(cafe => {
        ...
        return {
            name: cafe.name,
            address: `${place.street_no} ${place.locality}, CA ${place.postal_code}`,
...
function displayCafes(cafes) {
    ...
    cafes.forEach(cafe => {
        ...
        cafeDiv.innerHTML = `
            ...
            <p>Coordinates: ${cafe.coordinates}</p>
        `;
    ...
    if (cafes.length === 0) {
        resultDiv.innerHTML = 'No cafes found.';
    }
}

function searchCafes() {
    const searchTerm = document.getElementById('searchInput').value;
    const cafes = findCafes(searchTerm);
    displayCafes(cafes);
}

// Event Listeners
document.getElementById('searchButton').addEventListener('click', searchCafes);
